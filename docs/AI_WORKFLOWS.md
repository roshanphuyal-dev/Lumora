...
1–2. Same as above.
3. Orchestrator requests relevant chunks from NotebookLM for the target topic/scope (`TaskType.NOTEBOOK_QUERY`, same retrieval step as workflows 1/2). **Implemented:** if the quiz was created with `include_web_search=true` (`backend/app/models/quiz.py`), a second, independent retrieval also runs — `TaskType.INTERNET_SEARCH` with the same topic/title query (workflow 4's search), on success appended to the NotebookLM context behind a `--- Current web information ---` separator (or used directly if there was no notebook context) with its citations merged in; a search failure is swallowed the same way a `NOTEBOOK_QUERY` failure is, never failing generation over it (`app/workers/quiz_tasks.py`).
