...
`users` → `courses` → `subjects` → `documents` → `notebooks` (via `notebook_sources`) → `embeddings`. `quizzes` → `questions` → `quiz_attempts` → `weak_topics`/`progress`. Full ERD to be added once schema stabilizes.
