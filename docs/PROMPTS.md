...
| `teaching_explanation` v2 | Gemini / OpenCode Zen | One-shot notebook questions | prose shaped by accepted explanation depth/style when personalization is enabled |
| `quiz_generation` v2 | Gemini | Quiz workflow | JSON list covering all 8 question types; optionally requires exact easy/medium/hard counts selected from mastery when personalization is enabled; structured output, no fallback |
...
| `chat_response` v2 | Gemini | AI Chat workflow | markdown + citations, shaped by accepted explanation depth/style when personalization is enabled |
...
| `paper_search_synthesis` | Gemini | Paper Search workflow (ADR 0013) | free-text answer with inline `(source: <url>)` citations, synthesized from normalized arXiv/Semantic Scholar paper metadata (title/authors/venue/abstract) — never a provider summarization mode passed through |
