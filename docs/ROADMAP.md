...
Not Done per this doc's own Definition of Done below: neither new search/image feature is wired into quiz generation yet, research paper search isn't covered by either search provider, Tavily's privacy-policy-vs-FAQ ambiguity remains an open pre-production gate (`docs/SECURITY.md`, `ADR 0012`), and the quiz-taking UI has never been manually verified in a running browser (no connected Chrome extension during the milestones that built it) — shipped features are otherwise tested (257 passing backend/service tests) and their docs are current; frontend test coverage now exists for the quiz UI (Vitest/RTL infra, `QuizTakingView`/`AnswerReview` tests) but remains thin relative to the component surface, `docs/TESTING.md`.
