...
- **Search** — ambiguous between two distinct features as of ADR 0012: **Notebook Search** is retrieval scoped to a notebook's own indexed Sources (`docs/API.md`'s Notebook Search API); **Internet Search** is `TaskType.INTERNET_SEARCH` (ADR 0012, `docs/AI.md`) — a Tavily/Brave lookup against the open web, synthesized by Gemini, surfaced via `POST /notebooks/{id}/search` and the Ask tab's "Search the web" checkbox. Always qualify which one in code/docs/prompts; unqualified "search" is a code smell in this codebase now.
